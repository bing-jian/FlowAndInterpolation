...
void FlowInterpolation::findHoles(std::vector<int>& holes, DImage& vx,
                                  DImage& vy) {
...
void FlowInterpolation::fillHoles(std::vector<int>& holes, DImage& vx,
                                  DImage& vy) {
  const int nRows = vx.height();
  const int nCols = vx.width();
  const int size = holes.size();
  int neighbors[8] = {-nCols - 1, -nCols,    -nCols + 1, -1,
...
void FlowInterpolation::splatForward(DImage& vx, DImage& vy, DImage& pts,
                                     const DImage& vxForward,
                                     const DImage& vyForward, const DImage& Im1,
...
void FlowInterpolation::splatBackward(DImage& vx, DImage& vy, DImage& pts,
                                      const DImage& vxBackward,
                                      const DImage& vyBackward,
...
void FlowInterpolation::splatMotionsBidirect(
    DImage& vx, DImage& vy, const DImage& vxForward, const DImage& vyForward,
    const DImage& vxBackward, const DImage& vyBackward, const DImage& Im1,
    const DImage& Im2, double t) {
  const int nRows = vxForward.height();
  const int nCols = vxForward.width();
  DImage pts;
...
void FlowInterpolation::colorTransfer(DImage& dest, const DImage& interp,
                                      const DImage& Im1, const DImage& Im2,
                                      const DImage& forward,
                                      const DImage& backward, double t) {
