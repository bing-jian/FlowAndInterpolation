...
double CStochastic::GaussianSampling() {
  int i;
  double result = 0;
  for (i = 0; i < 12; i++) result += UniformSampling();
  result -= 6;
  return result;
}
...
double CStochastic::GetMean(double* signal, int length) {
  double mean = 0;
  int i;
  for (i = 0; i < length; i++) mean += signal[i];
  mean /= length;
  return mean;
}
...
int CStochastic::Sampling(double* Density, int NumSamples) {
  ...
  for (i = 0; i < NumSamples; i++) {
    sum += Density[i];
    if (sum >= RandNumber) return i;
  }
  return NumSamples - 1;
}

void CStochastic::Generate1DGaussian(double* pGaussian, int size,
                                     double sigma) {
  int i;
  if (sigma == 0) sigma = size / 2;
  for (i = -size; i <= size; i++)
    pGaussian[i + size] = exp(-(double)i * i / (2 * sigma));
}

void CStochastic::Generate2DGaussian(double* pGaussian, int WinSize,
                                     double sigma) {
  ...
  for (i = -WinSize; i <= WinSize; i++)
    for (j = -WinSize; j <= WinSize; j++)
      pGaussian[(i + WinSize) * WinLength + j + WinSize] =
          exp(-(double)(i * i + j * j) / (2 * Sigma));
...
double CStochastic::entropy(double* pDensity, int n) {
  double result = 0;
  int i;
  for (i = 0; i < n; i++) result -= log(__max(pDensity[i], 1E-6)) * pDensity[i];
  return result;
}
