...
class FlowInterpolation {
 public:
  static void splatMotionsBidirect(DImage& vx, DImage& vy,
                                   const DImage& vxForward,
                                   const DImage& vyForward,
                                   const DImage& vxBackward,
                                   const DImage& vyBackward, const DImage& Im1,
                                   const DImage& Im2, double t);
  static void splatForward(DImage& vx, DImage& vy, DImage& pts,
                           const DImage& vxForward, const DImage& vyForward,
                           const DImage& Im1, const DImage& Im2, double t);
  static void splatBackward(DImage& vx, DImage& vy, DImage& pts,
                            const DImage& vxBackward, const DImage& vyBackward,
                            const DImage& Im1, const DImage& Im2, double t);
  ...
  static void colorTransfer(DImage& dest, const DImage& interp,
                            const DImage& Im1, const DImage& Im2,
                            const DImage& forward, const DImage& backward,
                            double t);
  ...
  static inline bool insideBoundary(int h, int w, int row, int col,
                                    int neighbor) {
    if ((row == 0 && neighbor < -1) || (row == h - 1 && neighbor > 1) ||
        (col == 0 && neighbor % w == -1) ||
        (col == w - 1 && neighbor % w == 1)) {
      return false;
    }
    return true;
  }
  ...
  static inline double squareDistance(const double*& p1, const double*& p2,
                                      int len) {
    double sum = 0;
    double diff;
    for (int i = 0; i != len; ++i) {
  ...
  static inline double flowDistance(double fx, double fy, double ux,
                                    double uy) {
  ...
  static inline double intensity(const DImage& Im1, const DImage& Im2,
                                 double ux, double uy, int row, int col,
                                 double t) {
    int h = Im1.height();
    int w = Im1.width();
    int c = Im1.nchannels();
