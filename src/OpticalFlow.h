...
class OpticalFlow {
 ...
 public:
  ...
  static void SanityCheck(const DImage& imdx, const DImage& imdy,
  ...
  static void genInImageMask(DImage& mask, const DImage& vx, const DImage& vy,
                             int interval = 0);
  static void genInImageMask(DImage& mask, const DImage& flow,
                             int interval = 0);
  static void SmoothFlowPDE(const DImage& Im1, const DImage& Im2,
                            DImage& warpIm2, DImage& vx, DImage& vy,
                            double alpha, int nOuterFPIterations,
                            int nInnerFPIterations, int nCGIterations);

  static void SmoothFlowSOR(const DImage& Im1, const DImage& Im2,
                            DImage& warpIm2, DImage& vx, DImage& vy,
                            double alpha, int nOuterFPIterations,
                            int nInnerFPIterations, int nSORIterations);

  static void estGaussianMixture(const DImage& Im1, const DImage& Im2,
                                 GaussianMixture& para, double prior = 0.9);
  static void estLaplacianNoise(const DImage& Im1, const DImage& Im2,
                                Vector<double>& para);
  ...
  static void Coarse2FineFlow(DImage& vx, DImage& vy, DImage& warpI2,
                              const DImage& Im1, const DImage& Im2,
                              double alpha, double ratio, int minWidth,
                              int nOuterFPIterations, int nInnerFPIterations,
                              int nCGIterations);

  static void Coarse2FineFlowLevel(DImage& vx, DImage& vy, DImage& warpI2,
                                   const DImage& Im1, const DImage& Im2,
  ...
  static void ComputeOpticalFlow(const DImage& Im1, const DImage& Im2,
                                 DImage& flow) {
    if (!Im1.matchDimension(Im2)) {
      cout << "The input images for optical flow have different dimensions!"
           << endl;
      return;
    }
    ...
    DImage vx, vy, warpI2;
    OpticalFlow::Coarse2FineFlow(vx, vy, warpI2, Im1, Im2, alpha, ratio,
