...
// OpticalFlow::InterpolationMethod OpticalFlow::interpolation =
// OpticalFlow::Bicubic;
OpticalFlow::InterpolationMethod OpticalFlow::interpolation =
    OpticalFlow::Bilinear;
...
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::SanityCheck(const DImage &imdx, const DImage &imdy,
                              const DImage &imdt, double du, double dv) {
  if (imdx.matchDimension(imdy) == false ||
      imdx.matchDimension(imdt) == false) {
...
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::warpFL(DImage &warpIm2, const DImage &Im1, const DImage &Im2,
                         const DImage &vx, const DImage &vy) {
  if (warpIm2.matchDimension(Im2) == false)
    warpIm2.allocate(Im2.width(), Im2.height(), Im2.nchannels());
  ImageProcessing::warpImage(warpIm2.data(), Im1.data(), Im2.data(), vx.data(),
                             vy.data(), Im2.width(), Im2.height(),
                             Im2.nchannels());
}
...
void OpticalFlow::warpFL(DImage &warpIm2, const DImage &Im1, const DImage &Im2,
  ...
  if (warpIm2.matchDimension(Im2) == false)
    warpIm2.allocate(Im2.width(), Im2.height(), Im2.nchannels());
  ImageProcessing::warpImageFlow(warpIm2.data(), Im1.data(), Im2.data(),
                                 Flow.data(), Im2.width(), Im2.height(),
                                 Im2.nchannels());
}
...
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::genInImageMask(DImage &mask, const DImage &vx,
                                 const DImage &vy, int interval) {
  int imWidth, imHeight;
  imWidth = vx.width();
  imHeight = vx.height();
  if (mask.matchDimension(vx) == false) mask.allocate(imWidth, imHeight);
  ...
  for (int i = 0; i < imHeight; i++) {
    for (int j = 0; j < imWidth; j++) {
      ...
      y = i + pVx[offset];
      x = j + pVy[offset];
      if (x < interval || x > imWidth - 1 - interval || y < interval ||
          y > imHeight - 1 - interval)
        continue;
      pMask[offset] = 1;
    }
  }
...
void OpticalFlow::genInImageMask(DImage &mask, const DImage &flow,
                                 int interval) {
  int imWidth, imHeight;
  imWidth = flow.width();
...
//    warpIm2:                       the warped frame 2 according to the current
//    flow field u and v
//    u,v:                           the current flow field, NOTICE that they
//    are also output arguments
//
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::SmoothFlowSOR(const DImage &Im1, const DImage &Im2,
                                DImage &warpIm2, DImage &u, DImage &v,
                                double alpha, int nOuterFPIterations,
  ...
  for (int count = 0; count < nOuterFPIterations; count++) {
    ...
    // generate the mask to set the weight of the pxiels moving outside of the
    ...
    for (int hh = 0; hh < nInnerFPIterations; hh++) {
      ...
      for (int i = 0; i < nPixels; i++) {
        temp = uxData[i] * uxData[i] + uyData[i] * uyData[i] +
               vxData[i] * vxData[i] + vyData[i] * vyData[i];
      ...
      if (nChannels == 1)
        for (int i = 0; i < nPixels; i++) {
          temp =
              imdtData[i] + imdxData[i] * duData[i] + imdyData[i] * dvData[i];
          // if(temp*temp<0.04)
        ...
        for (int i = 0; i < nPixels; i++)
          for (int k = 0; k < nChannels; k++) {
            ...
            temp = imdtData[offset] + imdxData[offset] * duData[i] +
...
// Input arguments:
//     Im1, Im2:                        frame 1 and frame 2
//    warpIm2:                        the warped frame 2 according to the
//    current flow field u and v
//    u,v:                                    the current flow field, NOTICE
//    that they are also output arguments
//
//--------------------------------------------------------------------------------------------------------
void OpticalFlow::SmoothFlowPDE(const DImage &Im1, const DImage &Im2,
                                DImage &warpIm2, DImage &u, DImage &v,
                                double alpha, int nOuterFPIterations,
                                int nInnerFPIterations, int nCGIterations) {
  DImage mask, imdx, imdy, imdt;
  int imWidth, imHeight, nChannels, nPixels;
  imWidth = Im1.width();
  imHeight = Im1.height();
          ...
          for (int k = 0; k < nChannels; k++) {
            ...
            switch (noiseModel) {
              ...
              case Lap:
                if (LapPara[k] < 1E-20) continue;
                psiData[offset] =
                    1 / (2 * sqrt(temp + varepsilon_psi) * LapPara[k]);
                break;
            }
          }
...
void OpticalFlow::estGaussianMixture(const DImage &Im1, const DImage &Im2,
                                     GaussianMixture &para, double prior) {
  ...
  for (int count = 0; count < nIterations; count++) {
    ...
    for (int k = 0; k < nChannels; k++) {
      para.alpha[k] = total1[k] / (total1[k] + total2[k]) * (1 - prior) +
                      0.95 * prior;  // regularize alpha
      para.sigma[k] = sqrt(para.sigma[k] / total1[k]);
      para.beta[k] = sqrt(para.beta[k] / total2[k]) * (1 - prior) +
                     0.3 * prior;  // regularize beta
    }
    para.square();
  }
}

void OpticalFlow::estLaplacianNoise(const DImage &Im1, const DImage &Im2,
                                    Vector<double> &para) {
  ...
  for (int k = 0; k < nChannels; k++) total[k] = 0;
  ...
  for (int k = 0; k < nChannels; k++) {
    if (total[k] == 0) {
      cout << "All the pixels are invalid in estimation Laplacian noise!!!"
           << endl;
...
void OpticalFlow::Laplacian(DImage &output, const DImage &input,
  ...
  if (input.matchDimension(weight) == false) {
    cout << "Error in image dimension matching OpticalFlow::Laplacian()!"
         << endl;
    return;
  }
  ...
  for (int i = 0; i < height; i++)
    for (int j = 0; j < width - 1; j++) {
      ...
      fooData[offset] =
          (inputData[offset + 1] - inputData[offset]) * weightData[offset];
    }
  for (int i = 0; i < height; i++)
  ...
  for (int i = 0; i < height - 1; i++)
    for (int j = 0; j < width; j++) {
      int offset = i * width + j;
      fooData[offset] =
          (inputData[offset + width] - inputData[offset]) * weightData[offset];
    }
  for (int i = 0; i < height; i++)
    for (int j = 0; j < width; j++) {
      int offset = i * width + j;
...
void OpticalFlow::testLaplacian(int dim) {
  ...
  for (int i = 0; i < dim * dim; i++) {
    for (int j = 0; j < dim * dim; j++) {
      if (sysMatrix.data()[i * dim * dim + j] >= 0) printf(" ");
...
//--------------------------------------------------------------------------------------
void OpticalFlow::Coarse2FineFlow(DImage &vx, DImage &vy, DImage &warpI2,
                                  const DImage &Im1, const DImage &Im2,
  ...
  switch (noiseModel) {
    ...
    case Lap:
      ...
      for (int i = 0; i < LapPara.dim(); i++) LapPara[i] = 0.02;
      break;
  }
  ...
  for (int k = GPyramid1.nlevels() - 1; k >= 0; k--) {
    ...
    SmoothFlowSOR(Image1, Image2, WarpImage2, vx, vy, alpha,
                  nOuterFPIterations + k, nInnerFPIterations,
                  nCGIterations + k * 3);
...
void OpticalFlow::Coarse2FineFlowLevel(DImage &vx, DImage &vy, DImage &warpI2,
                                       const DImage &Im1, const DImage &Im2,
                                       double alpha, double ratio, int nLevels,
                                       int nOuterFPIterations,
  ...
  for (int k = GPyramid1.nlevels() - 1; k >= 0; k--) {
    ...
    SmoothFlowPDE(Image1, Image2, WarpImage2, vx, vy, alpha, nOuterFPIterations,
                  nInnerFPIterations, nCGIterations);
    // GMPara.display();
    if (IsDisplay) {
      cout << endl;
    }
  ...
  if (nchannels == 1) {
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        ...
        data[offset * 5 + 3] =
            im.data()[offset * 3 + 1] - im.data()[offset * 3];
        data[offset * 5 + 4] =
            im.data()[offset * 3 + 1] - im.data()[offset * 3 + 2];
        ++offset;
      }
    }
  } else {
...
bool OpticalFlow::SaveOpticalFlow(const DImage &flow, const char *filename) {
  ...
  for (int i = 0; i < flow.npixels(); i++) {
    foo.data()[i * 2] =
        (__min(__max(flow.data()[i * 2], -200), 200) + 200) * 160;
    foo.data()[i * 2 + 1] =
        (__min(__max(flow.data()[i * 2 + 1], -200), 200) + 200) * 160;
  }
