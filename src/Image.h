...
Image<T>::Image(int width, int height, int nchannels) {
  ...
  if (nElements > 0) memset(pData, 0, sizeof(T) * nElements);
  IsDerivativeImage = false;
}
...
template <class T>
void Image<T>::reset() {
  if (pData != NULL) memset(pData, 0, sizeof(T) * nElements);
}

template <class T>
void Image<T>::setValue(const T& value) {
  for (int i = 0; i < nElements; i++) pData[i] = value;
}

template <class T>
void Image<T>::setValue(const T& value, int _width, int _height,
                        int _nchannels) {
...
void Image<T>::copyData(const Image<T>& other) {
  ...
  if (nElements > 0) memcpy(pData, other.pData, sizeof(T) * nElements);
}
...
void Image<T>::copy(const Image<T1>& other) {
  ...
  for (int i = 0; i < nElements; i++) pData[i] = srcData[i];
}

template <class T>
void Image<T>::im2double() {
  if (IsFloat())
    for (int i = 0; i < nElements; i++) pData[i] /= 255;
}
...
//------------------------------------------------------------------------------------------
// function to move this image to a dest image at (x,y) with specified width and
// height
//------------------------------------------------------------------------------------------
template <class T>
template <class T1>
void Image<T>::moveto(Image<T1>& image, int x0, int y0, int width, int height) {
  if (width == 0) width = imWidth;
  if (height == 0) height = imHeight;
  ...
  for (int i = 0; i < height; i++) {
    ...
    for (int j = 0; j < width; j++) {
      ...
      for (int k = 0; k < NChannels; k++)
        image.data()[(y * image.width() + x) * image.nchannels() + k] =
            pData[(i * imWidth + j) * nChannels + k];
    }
  }
}
...
bool Image<T>::imresize(double ratio) {
  ...
  ImageProcessing::ResizeImage(pData, pDstData, imWidth, imHeight, nChannels,
                               ratio);
...
void Image<T>::imresize(Image<T1>& result, double ratio) const {
  ...
  if (result.width() != DstWidth || result.height() != DstHeight ||
    ...
    result.reset();
  ImageProcessing::ResizeImage(pData, result.data(), imWidth, imHeight,
                               nChannels, ratio);
}
...
void Image<T>::imresize(Image<T1>& result, int DstWidth, int DstHeight) const {
  if (result.width() != DstWidth || result.height() != DstHeight ||
    ...
    result.reset();
  ImageProcessing::ResizeImage(pData, result.data(), imWidth, imHeight,
                               nChannels, DstWidth, DstHeight);
}

template <class T>
void Image<T>::imresize(int dstWidth, int dstHeight) {
  DImage foo(dstWidth, dstHeight, nChannels);
  ImageProcessing::ResizeImage(pData, foo.data(), imWidth, imHeight, nChannels,
                               dstWidth, dstHeight);
  copyData(foo);
}
...
bool Image<T>::loadImage(ifstream& myfile) {
  ...
  myfile.read(type, 16);
#ifdef _LINUX_MAC
  if (strcasecmp(type, "uint16") == 0) sprintf(type, "unsigned short");
  if (strcasecmp(type, "uint32") == 0) sprintf(type, "unsigned int");
  if (strcasecmp(type, typeid(T).name()) != 0) {
    cout << "The type of the image is different from the type of the object!"
         << endl;
    return false;
  }
#else
  if (strcmpi(type, "uint16") == 0) sprintf(type, "unsigned short");
  if (strcmpi(type, "uint32") == 0) sprintf(type, "unsigned int");
...
//     {
//         computeDimension();
//         colorType = BGR; // when we use qt or opencv to load the image, it's
//         often BGR
//         return true;
//     }
//     return false;
...
//     return
//     ImageIO::saveImage(filename,pData,imWidth,imHeight,nChannels,type);
// }
...
// template <class T>
// bool Image<T>::imwrite(const QString &filename, ImageIO::ImageType imagetype,
// int quality) const
//{
//    return ImageIO::writeImage(filename,(const
//    T*&)pData,imWidth,imHeight,nChannels,imagetype,quality);
//}
//
// template <class T>
// bool Image<T>::imwrite(const QString &filename, T min, T max, int quality)
// const
//{
//    return ImageIO::writeImage(filename,(const
//    T*&)pData,imWidth,imHeight,nChannels,min,max,quality);
//}
...
void Image<T>::dx(Image<T1>& result, bool IsAdvancedFilter) const {
  ...
  if (IsAdvancedFilter == false) {
    ...
    for (int i = 0; i < 5; i++) xFilter[i] /= 12;
    ImageProcessing::hfiltering(pData, data, imWidth, imHeight, nChannels,
                                xFilter, 2);
  }
}
...
void Image<T>::dy(Image<T1>& result, bool IsAdvancedFilter) const {
  ...
  if (IsAdvancedFilter == false) {
    ...
    for (int i = 0; i < 5; i++) yFilter[i] /= 12;
    ImageProcessing::vfiltering(pData, data, imWidth, imHeight, nChannels,
                                yFilter, 2);
  }
}
...
void Image<T>::dxx(Image<T1>& image) const {
  if (!matchDimension(image)) image.allocate(imWidth, imHeight, nChannels);
  ...
  if (nChannels == 1)  // if there is only one image channel
    for (int i = 0; i < imHeight; i++)
      for (int j = 0; j < imWidth; j++) {
        ...
        pDstData[offset] =
            pData[offset] * 2 - pData[offset - 1] - pData[offset + 1];
      }
  else
    for (int i = 0; i < imHeight; i++)
      for (int j = 0; j < imWidth; j++) {
        ...
        if (j == 0) {
          for (int k = 0; k < nChannels; k++)
            pDstData[offset + k] =
                pData[offset + k] - pData[offset + nChannels + k];
          continue;
        }
        if (j == imWidth - 1) {
          for (int k = 0; k < nChannels; k++)
        ...
        for (int k = 0; k < nChannels; k++)
          pDstData[offset + k] = pData[offset + k] * 2 -
                                 pData[offset + nChannels + k] -
                                 pData[offset - nChannels + k];
      }
}
  ...
  if (nChannels == 1)
    for (int i = 0; i < imHeight; i++)
      for (int j = 0; j < imWidth; j++) {
        ...
        pDstData[offset] = pData[offset] * 2 - pData[offset + imWidth] -
                           pData[offset - imWidth];
      }
  else
    for (int i = 0; i < imHeight; i++)
      for (int j = 0; j < imWidth; j++) {
        ...
        if (i == 0) {
          for (int k = 0; k < nChannels; k++)
            pDstData[offset + k] =
                pData[offset + k] - pData[offset + imWidth * nChannels + k];
          continue;
        }
        if (i == imHeight - 1) {
          for (int k = 0; k < nChannels; k++)
        ...
        for (int k = 0; k < nChannels; k++)
          pDstData[offset + k] = pData[offset + k] * 2 -
                                 pData[offset + imWidth * nChannels + k] -
                                 pData[offset - imWidth * nChannels + k];
      }
}
...
void Image<T>::laplacian(Image<T1>& image) const {
  if (!matchDimension(image)) image.allocate(*this);
...
template <class T>
template <class T1>
void Image<T>::GaussianSmoothing(Image<T1>& image, double sigma,
                                 int fsize) const {
  Image<T1> foo;
  ...
  for (int i = 0; i < 2 * fsize + 1; i++) gFilter[i] /= sum;
...
template <class T>
template <class T1>
void Image<T>::GaussianSmoothing_transpose(Image<T1>& image, double sigma,
                                           int fsize) const {
  Image<T1> foo;
  // constructing the 1D gaussian filter
  double* gFilter;
...
void Image<T>::smoothing(Image<T1>& image, double factor) {
  ...
  for (int i = 0; i < 9; i++) filter2D[i] /= (factor + 2) * (factor + 2);
...
template <class T>
template <class T1>
void Image<T>::imfilter(Image<T1>& image, const double* filter,
                        int fsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::filtering(pData, image.data(), imWidth, imHeight, nChannels,
                             filter, fsize);
}
...
void Image<T>::imfilter(Image<T1>& image, const Image<T2>& kernel) const {
  if (kernel.width() != kernel.height()) {
    cout << "Error in Image<T>::imfilter(Image<T1>& image,const Image<T2>& "
            "kernel)"
         << endl;
    exit(-1);
  }
...
void Image<T>::imfilter_h(Image<T1>& image, double* filter, int fsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::hfiltering(pData, image.data(), imWidth, imHeight, nChannels,
                              filter, fsize);
}
...
void Image<T>::imfilter_v(Image<T1>& image, double* filter, int fsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::vfiltering(pData, image.data(), imWidth, imHeight, nChannels,
                              filter, fsize);
}

template <class T>
template <class T1>
void Image<T>::imfilter_hv(Image<T1>& image, const double* hfilter, int hfsize,
                           const double* vfilter, int vfsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  T1* pTempBuffer;
  pTempBuffer = new T1[nElements];
  ImageProcessing::hfiltering(pData, pTempBuffer, imWidth, imHeight, nChannels,
                              hfilter, hfsize);
  ImageProcessing::vfiltering(pTempBuffer, image.data(), imWidth, imHeight,
                              nChannels, vfilter, vfsize);
  delete[] pTempBuffer;
}

template <class T>
template <class T1>
void Image<T>::imfilter_hv(Image<T1>& image, const Image<double>& hfilter,
                           const Image<double>& vfilter) const {
...
template <class T>
template <class T1>
void Image<T>::imfilter_transpose(Image<T1>& image, const double* filter,
                                  int fsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::filtering_transpose(pData, image.data(), imWidth, imHeight,
                                       nChannels, filter, fsize);
}

template <class T>
template <class T1, class T2>
void Image<T>::imfilter_transpose(Image<T1>& image,
                                  const Image<T2>& kernel) const {
  if (kernel.width() != kernel.height()) {
    cout << "Error in Image<T>::imfilter(Image<T1>& image,const Image<T2>& "
            "kernel)"
         << endl;
...
template <class T>
template <class T1>
void Image<T>::imfilter_h_transpose(Image<T1>& image, double* filter,
                                    int fsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::hfiltering_transpose(pData, image.data(), imWidth, imHeight,
                                        nChannels, filter, fsize);
}

template <class T>
template <class T1>
void Image<T>::imfilter_v_transpose(Image<T1>& image, double* filter,
                                    int fsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::vfiltering_transpose(pData, image.data(), imWidth, imHeight,
                                        nChannels, filter, fsize);
}

template <class T>
template <class T1>
void Image<T>::imfilter_hv_transpose(Image<T1>& image, const double* hfilter,
                                     int hfsize, const double* vfilter,
                                     int vfsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  ...
  // imwrite("input.jpg");
  ImageProcessing::vfiltering_transpose(pData, temp.data(), imWidth, imHeight,
                                        nChannels, vfilter, vfsize);
  // temp.imwrite("temp.jpg");
  ImageProcessing::hfiltering_transpose(temp.data(), image.data(), imWidth,
                                        imHeight, nChannels, hfilter, hfsize);
}

template <class T>
template <class T1>
void Image<T>::imfilter_hv_transpose(Image<T1>& image,
                                     const Image<double>& hfilter,
                                     const Image<double>& vfilter) const {
  int hfsize = (__max(hfilter.width(), hfilter.height()) - 1) / 2;
  int vfsize = (__max(vfilter.width(), vfilter.height()) - 1) / 2;
...
void Image<T>::desaturate(Image<T1>& image) const {
  ...
  for (int i = 0; i < nPixels; i++) {
    if (colorType == RGB) {
      data[i] =
          static_cast<double>(pData[offset] * .299 + pData[offset + 1] * .587 +
                              pData[offset + 2] * .114);
    } else {
      data[i] =
          static_cast<double>(pData[offset] * .114 + pData[offset + 1] * .587 +
                              pData[offset + 2] * .299);
    }
    offset += 3;
  }
...
template <class T>
template <class T1, class T2>
void Image<T>::concatenate(Image<T1>& destImage,
                           const Image<T2>& addImage) const {
  if (addImage.width() != imWidth || addImage.height() != imHeight) {
    destImage.copy(*this);
    return;
  }
  int extNChannels = nChannels + addImage.nchannels();
  if (destImage.width() != imWidth || destImage.height() != imHeight ||
      destImage.nchannels() != extNChannels)
  ...
  for (int i = 0; i < imHeight; i++)
    for (int j = 0; j < imWidth; j++) {
      ...
      for (int k = nChannels; k < extNChannels; k++)
        pDestData[offset * extNChannels + k] =
            pAddData[offset * addImage.nchannels() + k - nChannels];
    }
}

template <class T>
template <class T1, class T2>
void Image<T>::concatenate(Image<T1>& destImage, const Image<T2>& addImage,
                           double ratio) const {
  if (addImage.width() != imWidth || addImage.height() != imHeight) {
    destImage.copy(*this);
    return;
  ...
  for (int i = 0; i < imHeight; i++) {
    for (int j = 0; j < imWidth; j++) {
      ...
      for (int k = nChannels; k < extNChannels; k++) {
        pDestData[offset * extNChannels + k] =
            pAddData[offset * addImage.nchannels() + k - nChannels] * ratio;
      }
      ++offset;
    }
  }
...
template <class T>
template <class T1, class T2>
void Image<T>::separate(unsigned int firstNChannels, Image<T1>& image1,
                        Image<T2>& image2) const {
  ...
  if (image1.width() != imWidth || image1.height() != imHeight ||
      image1.nchannels() != firstNChannels)
    image1.allocate(imWidth, imHeight, firstNChannels);
  if (image2.width() != imWidth || image2.height() != imHeight ||
      image2.nchannels() != secondNChannels)
  ...
  for (int i = 0; i < imHeight; i++)
    for (int j = 0; j < imWidth; j++) {
      for (int k = 0; k < firstNChannels; k++)
        image1.pData[offset * firstNChannels + k] =
            pData[offset * nChannels + k];
      for (int k = firstNChannels; k < nChannels; k++)
        image2.pData[offset * secondNChannels + k - firstNChannels] =
            pData[offset * nChannels + k];
      ++offset;
    }
}
...
void Image<T>::getPatch(Image<T1>& patch, double x, double y, int wsize) const {
  ...
  if (patch.width() != wlength || patch.height() != wlength ||
      patch.nchannels() != nChannels)
    patch.allocate(wlength, wlength, nChannels);
  else
    patch.reset();
  ImageProcessing::getPatch(pData, patch.data(), imWidth, imHeight, nChannels,
                            x, y, wsize);
}
...
template <class T>
template <class T1>
void Image<T>::crop(Image<T1>& patch, int Left, int Top, int Width,
                    int Height) const {
  ...
  if (Width < 0 || Height < 0 || Width + Left > imWidth ||
      Height + Top > imHeight) {
    cout << "The patch to crop is invalid!" << endl;
    return;
  }
  ImageProcessing::cropImage(pData, imWidth, imHeight, nChannels, patch.data(),
                             Left, Top, Width, Height);
}

template <class T>
void Image<T>::flip_horizontal(Image<T>& image) {
  if (!image.matchDimension(*this)) image.allocate(*this);
  for (int i = 0; i < imHeight; i++)
    for (int j = 0; j < imWidth; j++) {
...
void Image<T>::Multiply(const Image<T1>& image1, const Image<T2>& image2,
  ...
  if (image1.matchDimension(image2) == false ||
      image2.matchDimension(image3) == false) {
    cout << "Error in image dimensions--function Image<T>::Multiply()!" << endl;
    return;
  }
  if (matchDimension(image1) == false) allocate(image1);
...
void Image<T>::Multiply(const Image<T1>& image1, const Image<T2>& image2) {
  ...
  for (int i = 0; i < nElements; i++) pData[i] = pData1[i] * pData2[i];
}

template <class T>
template <class T1, class T2>
void Image<T>::MultiplyAcross(const Image<T1>& image1,
...
void Image<T>::Multiplywith(const Image<T1>& image1) {
  if (matchDimension(image1) == false) {
    cout << "Error in image dimensions--function Image<T>::Multiplywith()!"
         << endl;
    return;
  }
  const T1*& pData1 = image1.data();
  for (int i = 0; i < nElements; i++) pData[i] *= pData1[i];
}

template <class T>
template <class T1>
void Image<T>::MultiplywithAcross(const Image<T1>& image1) {
  if (imWidth != image1.width() || imHeight != image1.height() ||
      image1.nchannels() != 1) {
    cout
        << "Error in image dimensions--function Image<T>::MultiplywithAcross()!"
        << endl;
    return;
  }
  const T1*& pData1 = image1.data();
  for (int i = 0; i < nPixels; i++)
    for (int j = 0; j < nChannels; j++) pData[i * nChannels + j] *= pData1[i];
}
...
void Image<T>::Add(const Image<T1>& image1, const Image<T2>& image2,
  ...
  const T1*& pData1 = image1.data();
  const T2*& pData2 = image2.data();
  for (int i = 0; i < nElements; i++) pData[i] = pData1[i] + pData2[i] * ratio;
}
...
void Image<T>::Add(const Image<T1>& image1, const double ratio) {
  ...
  const T1*& pData1 = image1.data();
  for (int i = 0; i < nElements; i++) pData[i] += pData1[i] * ratio;
}
...
void Image<T>::Subtract(const Image<T1>& image1, const Image<T2>& image2) {
  ...
  const T1*& pData1 = image1.data();
  const T2*& pData2 = image2.data();
  for (int i = 0; i < nElements; i++) pData[i] = (T)pData1[i] - pData2[i];
}
...
template <class T>
void Image<T>::square() {
  for (int i = 0; i < nElements; i++) pData[i] = pData[i] * pData[i];
}
...
void Image<T>::Exp(double sigma) {
  for (int i = 0; i < nElements; i++) pData[i] = exp(-pData[i] / sigma);
}
...
double Image<T>::norm2() const {
  double result = 0;
  for (int i = 0; i < nElements; i++) result += pData[i] * pData[i];
  return result;
}
...
double Image<T>::sum() const {
  double result = 0;
  for (int i = 0; i < nElements; i++) result += pData[i];
  return result;
}
...
double Image<T>::innerproduct(Image<T1>& image) const {
  ...
  for (int i = 0; i < nElements; i++) result += pData[i] * pData1[i];
  return result;
}
...
//    if(imWidth<0 ||imWidth>100000 || imHeight<0 || imHeight>100000 ||
//    nChannels<0 || nChannels>10000)
//        return false;
...
template <class T>
template <class T1>
void Image<T>::BilateralFiltering(Image<T1>& other, int fsize,
                                  double filter_sigma, double range_sigma) {
  double* pBuffer;
  ...
  for (int i = -fsize; i <= fsize; i++)
    for (int j = -fsize; j <= fsize; j++)
      pSpatialWeight[(i + fsize) * flength + j + fsize] =
          exp(-(double)(i * i + j * j) / (2 * filter_sigma * filter_sigma));

  for (int i = 0; i < imHeight; i++)
    for (int j = 0; j < imWidth; j++) {
      double totalWeight = 0;
      for (int k = 0; k < other.nchannels(); k++) pBuffer[k] = 0;
      for (int ii = -fsize; ii <= fsize; ii++)
        for (int jj = -fsize; jj <= fsize; jj++) {
          ...
          for (int k = 0; k < nChannels; k++) {
            double diff =
                pData[offset + k] - pData[(i * imWidth + j) * nChannels + k];
            temp += diff * diff;
          }
          ...
          for (int k = 0; k < other.nchannels(); k++)
            pBuffer[k] +=
                other.data()[(y * imWidth + x) * other.nchannels() + k] *
                weight;
        }
      for (int k = 0; k < other.nchannels(); k++)
        result.data()[(i * imWidth + j) * other.nchannels()] =
            pBuffer[k] / totalWeight;
    }
...
template <class T>
// Image<T>  Image<T>::BilateralFiltering(int fsize,double filter_sigma,double
// range_sigma)
void Image<T>::imBilateralFiltering(Image<T>& result, int fsize,
                                    double filter_sigma, double range_sigma) {
  // Image<T> result(*this);
  ...
  for (int i = 0; i < imHeight; i++)
    for (int j = 0; j < imWidth; j++) {
      double totalWeight = 0;
      for (int k = 0; k < nChannels; k++) pBuffer[k] = 0;
...
template <class T>
template <class T1, class T2>
int Image<T>::kmeansIndex(int pixelIndex, T1& MinDistance,
                          const T2* pDictionary, int nVocabulary, int nDim) {
  ...
  for (int j = 0; j < nVocabulary; j++) {
    ...
    for (int k = 0; k < nDim; k++)
      Distance += (T1)(pData[offset1 + k] - pDictionary[offset2 + k]) *
                  (pData[offset1 + k] - pDictionary[offset2 + k]);
    if (j == 0) {
...
template <class T>
template <class T1, class T2>
void Image<T>::ConvertToVisualWords(Image<T1>& result, const T2* pDictionary,
                                    int nDim, int nVocabulary) {
  if (nChannels != nDim) {
    cout << "The dimension of the vocabulary must match to the nChannels of "
            "the image"
         << endl;
    return;
  }
...
// notice that the range of the coordinate is [0,imWidth] (x) and [0,imHeight]
// (y)
template <class T>
template <class T1>
Vector<T1> Image<T>::histogramRegion(int nBins, double left, double top,
  ...
  for (int i = Top; i <= Bottom; i++)
    for (int j = Left; j <= Right; j++) {
      ...
      if (Top == Bottom)
        ...
        if (i == Top) weight *= dTop;
        if (i == Bottom) weight *= dBottom;
      }

      if (Left == Right)
        weight *= (dLeft + dRight - 1);
      else {
        if (j == Left) weight *= dLeft;
        if (j == Right) weight *= dRight;
      }
...
template <class T>
template <class T1>
void Image<T>::warpImageBicubic(Image<T>& output, const Image<T1>& vx,
                                const Image<T1>& vy) const {
...
template <class T>
template <class T1, class T2>
void Image<T>::warpImageBicubic(Image<T>& output, const Image<T1>& imdx,
                                const Image<T1>& imdy, const Image<T1>& imdxdy,
                                const Image<T2>& vx,
                                const Image<T2>& vy) const {
  T* pIm = pData;
  ...
  for (int i = 0; i < height; i++)
    for (int j = 0; j < width; j++) {
      ...
      for (int k = 0; k < nChannels; k++) {
        ...
        output.pData[offset * nChannels + k] =
            a[0][0] + a[0][1] * dy + a[0][2] * dy2 + a[0][3] * dy3 +
            a[1][0] * dx + a[1][1] * dx * dy + a[1][2] * dx * dy2 +
            a[1][3] * dx * dy3 + a[2][0] * dx2 + a[2][1] * dx2 * dy +
            a[2][2] * dx2 * dy2 + a[2][3] * dx2 * dy3 + a[3][0] * dx3 +
            a[3][1] * dx3 * dy + a[3][2] * dx3 * dy2 + a[3][3] * dx3 * dy3;
        // output.pData[offset*nChannels+k] =
        // __max(__min(output.pData[offset*nChannels+k],ImgMax),0);
      }
    }
}
...
template <class T>
template <class T1, class T2>
void Image<T>::warpImageBicubic(Image<T>& output, const Image<T1>& coeff,
                                const Image<T2>& vx,
                                const Image<T2>& vy) const {
  T* pIm = pData;
  int width = vx.width();
      ...
      for (int k = 0; k < nChannels; k++) {
        ...
        for (int ii = 0; ii < 4; ii++)
          for (int jj = 0; jj < 4; jj++)
            a[ii][jj] =
                coeff.pData[(offset * nChannels + k) * 16 + ii * 4 + jj];
...
template <class T>
template <class T1>
void Image<T>::BicubicCoeff(double a[][4], const T* pIm, const T1* pImDx,
                            const T1* pImDy, const T1* pImDxDy,
                            const int offsets[][2]) const {
  a[0][0] = pIm[offsets[0][0]];
  a[1][0] = pImDx[offsets[0][0]];
  a[2][0] = -3 * pIm[offsets[0][0]] + 3 * pIm[offsets[1][0]] -
            2 * pImDx[offsets[0][0]] - pImDx[offsets[1][0]];
  a[3][0] = 2 * pIm[offsets[0][0]] - 2 * pIm[offsets[1][0]] +
            pImDx[offsets[0][0]] + pImDx[offsets[1][0]];

  a[0][1] = pImDy[offsets[0][0]];
  a[1][1] = pImDxDy[offsets[0][0]];
  a[2][1] = -3 * pImDy[offsets[0][0]] + 3 * pImDy[offsets[1][0]] -
            2 * pImDxDy[offsets[0][0]] - pImDxDy[offsets[1][0]];
  a[3][1] = 2 * pImDy[offsets[0][0]] - 2 * pImDy[offsets[1][0]] +
            pImDxDy[offsets[0][0]] + pImDxDy[offsets[1][0]];

  a[0][2] = -3 * pIm[offsets[0][0]] + 3 * pIm[offsets[0][1]] -
            2 * pImDy[offsets[0][0]] - pImDy[offsets[0][1]];
  a[1][2] = -3 * pImDx[offsets[0][0]] + 3 * pImDx[offsets[0][1]] -
            2 * pImDxDy[offsets[0][0]] - pImDxDy[offsets[0][1]];
  a[2][2] = 9 * pIm[offsets[0][0]] - 9 * pIm[offsets[1][0]] -
            9 * pIm[offsets[0][1]] + 9 * pIm[offsets[1][1]] +
            6 * pImDx[offsets[0][0]] + 3 * pImDx[offsets[1][0]] -
            6 * pImDx[offsets[0][1]] - 3 * pImDx[offsets[1][1]] +
            6 * pImDy[offsets[0][0]] - 6 * pImDy[offsets[1][0]] +
            3 * pImDy[offsets[0][1]] - 3 * pImDy[offsets[1][1]] +
            4 * pImDxDy[offsets[0][0]] + 2 * pImDxDy[offsets[1][0]] +
            2 * pImDxDy[offsets[0][1]] + pImDxDy[offsets[1][1]];
  a[3][2] = -6 * pIm[offsets[0][0]] + 6 * pIm[offsets[1][0]] +
            6 * pIm[offsets[0][1]] - 6 * pIm[offsets[1][1]] +
            (-3) * pImDx[offsets[0][0]] - 3 * pImDx[offsets[1][0]] +
            3 * pImDx[offsets[0][1]] + 3 * pImDx[offsets[1][1]] +
            (-4) * pImDy[offsets[0][0]] + 4 * pImDy[offsets[1][0]] -
            2 * pImDy[offsets[0][1]] + 2 * pImDy[offsets[1][1]] +
            (-2) * pImDxDy[offsets[0][0]] - 2 * pImDxDy[offsets[1][0]] -
            pImDxDy[offsets[0][1]] - pImDxDy[offsets[1][1]];

  a[0][3] = 2 * pIm[offsets[0][0]] - 2 * pIm[offsets[0][1]] +
            pImDy[offsets[0][0]] + pImDy[offsets[0][1]];
  a[1][3] = 2 * pImDx[offsets[0][0]] - 2 * pImDx[offsets[0][1]] +
            pImDxDy[offsets[0][0]] + pImDxDy[offsets[0][1]];
  a[2][3] = -6 * pIm[offsets[0][0]] + 6 * pIm[offsets[1][0]] +
            6 * pIm[offsets[0][1]] - 6 * pIm[offsets[1][1]] +
            (-4) * pImDx[offsets[0][0]] - 2 * pImDx[offsets[1][0]] +
            4 * pImDx[offsets[0][1]] + 2 * pImDx[offsets[1][1]] +
            (-3) * pImDy[offsets[0][0]] + 3 * pImDy[offsets[1][0]] -
            3 * pImDy[offsets[0][1]] + 3 * pImDy[offsets[1][1]] +
            (-2) * pImDxDy[offsets[0][0]] - pImDxDy[offsets[1][0]] -
            2 * pImDxDy[offsets[0][1]] - pImDxDy[offsets[1][1]];
  a[3][3] = 4 * pIm[offsets[0][0]] - 4 * pIm[offsets[1][0]] -
            4 * pIm[offsets[0][1]] + 4 * pIm[offsets[1][1]] +
            2 * pImDx[offsets[0][0]] + 2 * pImDx[offsets[1][0]] -
            2 * pImDx[offsets[0][1]] - 2 * pImDx[offsets[1][1]] +
            2 * pImDy[offsets[0][0]] - 2 * pImDy[offsets[1][0]] +
            2 * pImDy[offsets[0][1]] - 2 * pImDy[offsets[1][1]] +
            pImDxDy[offsets[0][0]] + pImDxDy[offsets[1][0]] +
            pImDxDy[offsets[0][1]] + pImDxDy[offsets[1][1]];
}
  ...
  for (int i = 0; i < imHeight; i++)
    for (int j = 0; j < imWidth; j++) {
      ...
      for (int k = 0; k < nChannels; k++) {
        ...
        for (int ii = 0; ii < 4; ii++)
          for (int jj = 0; jj < 4; jj++)
            output.pData[(offset * nChannels + k) * 16 + ii * 4 + jj] =
                a[ii][jj];
      }
    }
}
...
template <class T>
template <class T1>
void Image<T>::warpImageBicubicRef(const Image<T>& ref, Image<T>& output,
                                   const Image<T1>& vx,
                                   const Image<T1>& vy) const {
  double dfilter[3] = {-0.5, 0, 0.5};
  DImage imdx, imdy, imdxdy;
  ...
  for (int i = 0; i < height; i++)
    for (int j = 0; j < width; j++) {
      ...
      for (int k = 0; k < nChannels; k++) {
        ...
        // output.pData[offset*nChannels+k] =
        // __max(__min(output.pData[offset*nChannels+k],ImgMax),0);
        // if(!(output.pData[offset*nChannels+k]<100000 &&
        // output.pData[offset*nChannels+k]>-100000)) // bound the values
...
template <class T>
template <class T1>
void Image<T>::warpImage(Image<T>& output, const Image<T1>& vx,
                         const Image<T1>& vy) const {
  if (!output.matchDimension(*this)) output.allocate(*this);
  ImageProcessing::warpImage(output.data(), pData, vx.data(), vy.data(),
                             imWidth, imHeight, nChannels);
}

template <class T>
template <class T1>
void Image<T>::warpImage_transpose(Image<T>& output, const Image<T1>& vx,
                                   const Image<T1>& vy) const {
  if (!output.matchDimension(*this)) output.allocate(*this);
  ImageProcessing::warpImage_transpose(output.data(), pData, vx.data(),
                                       vy.data(), imWidth, imHeight, nChannels);
}
...
void Image<T>::warpImage(Image<T>& output, const Image<T1>& flow) const {
  if (!output.matchDimension(*this)) output.allocate(*this);
  ImageProcessing::warpImage(output.data(), pData, flow.data(), imWidth,
                             imHeight, nChannels);
}

template <class T>
template <class T1>
void Image<T>::warpImage_transpose(Image<T>& output,
                                   const Image<T1>& flow) const {
  if (!output.matchDimension(*this)) output.allocate(*this);
  ImageProcessing::warpImage_transpose(output.data(), pData, flow.data(),
                                       imWidth, imHeight, nChannels);
}
...
T Image<T>::max() const {
  T Max = pData[0];
  for (int i = 0; i < nElements; i++) Max = __max(Max, pData[i]);
  return Max;
}
...
T Image<T>::min() const {
  T Min = pData[0];
  for (int i = 0; i < nElements; i++) Min = __min(Min, pData[i]);
  return Min;
}
...
template <class T>
template <class T1>
void Image<T>::LoadMatlabImageCore(const mxArray* image,
                                   bool IsImageScaleCovnersion) {
  ...
  if (isfloat == true)
    for (int i = 0; i < imHeight; i++)
      for (int j = 0; j < imWidth; j++)
        for (int k = 0; k < nChannels; k++)
          pData[offset++] =
              (double)pMatlabPlane[k * nPixels + j * imHeight + i] / 255;
  else
    for (int i = 0; i < imHeight; i++)
      for (int j = 0; j < imWidth; j++)
        for (int k = 0; k < nChannels; k++)
...
template <class T>
bool Image<T>::LoadMatlabImage(const mxArray* matrix,
                               bool IsImageScaleCovnersion) {
...
template <class T>
template <class T1>
void Image<T>::ConvertFromMatlab(const T1* pMatlabPlane, int _width,
                                 int _height, int _nchannels) {
  if (imWidth != _width || imHeight != _height || nChannels != _nchannels)
    allocate(_width, _height, _nchannels);
  int offset = 0;
  for (int i = 0; i < imHeight; i++)
