...
#define _Release_2DArray(X, i, length) \
  for (i = 0; i < length; i++)         \
...
class CStochastic {
 public:
  ...
  template <class T>
  static void GetMeanVar(T* signal, int length, double* mean, double* var);
  ...
  template <class T>
  static T sum(int NumData, T* pData);
  template <class T>
  static void Normalize(int NumData, T* pData);
  template <class T>
  static T mean(int NumData, T* pData);
  template <class T>
  static void sort(int number, T* pData, int* pIndex,
                   SortType m_SortType = SortDescending);
  template <class T>
  static T Min(int NumData, T* pData);
  template <class T>
  static T Min(int NumData, T* pData1, T* pData2);
  template <class T>
  static T Max(int NumData, T* pData);
  template <class T>
  static int FindMax(int NumData, T* pData);
  template <class T1, class T2>
  static void ComputeVectorMean(int Dim, int NumData, T1* pData, T2* pMean,
                                double* pWeight = NULL);
  template <class T1, class T2>
  static void ComputeMeanCovariance(int Dim, int NumData, T1* pData, T2* pMean,
                                    T2* pCovarance, double* pWeight = NULL);
  template <class T1, class T2>
  static double VectorSquareDistance(int Dim, T1* pVector1, T2* pVector2);
  template <class T1>
  static void KMeanClustering(int Dim, int NumData, int NumClusters, T1* pData,
                              int* pPartition, double** pClusterMean = NULL,
                              int MaxIterationNum = 10,
                              int MinClusterSampleNumber = 2);
  template <class T>
  static double norm(T* X, int Dim);
  template <class T1, class T2>
  static int FindClosestPoint(T1* pPointSet, int NumPoints, int nDim,
                              T2* QueryPoint);
  template <class T1, class T2>
  static void GaussianFiltering(T1* pSrcArray, T2* pDstArray, int NumPoints,
                                int nChannels, int size, double sigma);
};
...
T CStochastic::sum(int NumData, T* pData) {
  T sum = 0;
  int i;
  for (i = 0; i < NumData; i++) sum += pData[i];
  return sum;
}
...
void CStochastic::Normalize(int NumData, T* pData) {
  ...
  for (i = 0; i < NumData; i++) pData[i] /= Sum;
}
...
template <class T1, class T2>
void CStochastic::ComputeMeanCovariance(int Dim, int NumData, T1* pData,
...
template <class T1, class T2>
void CStochastic::ComputeVectorMean(int Dim, int NumData, T1* pData, T2* pMean,
                                    double* pWeight) {
  int i, j;
  memset(pMean, 0, sizeof(T2) * Dim);
  bool IsWeightLoaded;
...
template <class T1>
void CStochastic::KMeanClustering(int Dim, int NumData, int NumClusters,
                                  T1* pData, int* pPartition,
                                  double** pClusterMean, int MaxIterationNum,
                                  int MinClusterSampleNumber) {
  int i, j, k, l, Index, ClusterSampleNumber;
  double MinDistance, Distance;
  double** pCenters;
  pCenters = new double*[NumClusters];
  for (i = 0; i < NumClusters; i++) pCenters[i] = new double[Dim];
...
template <class T1, class T2>
int CStochastic::FindClosestPoint(T1* pPointSet, int NumPoints, int nDim,
                                  T2* QueryPoint) {
...
template <class T1, class T2>
void CStochastic::GaussianFiltering(T1* pSrcArray, T2* pDstArray, int NumPoints,
                                    int nChannels, int size, double sigma) {
  int i, j, u, l;
