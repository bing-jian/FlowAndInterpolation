...
// --------------------------------------------------------------------------------------------------
template <class T1, class T2>
inline void ImageProcessing::BilinearInterpolate(const T1* pImage, int width,
                                                 int height, int nChannels,
                                                 double x, double y,
                                                 T2* result) {
  ...
  yy = y;
  double dx, dy, s;
  dx = std::max(std::min(x - xx, 1.), 0.);
  dy = std::max(std::min(y - yy, 1.), 0.);
...
// nChannels
// bilinear interpolation is used for now. It might be replaced by other
