...
// if the files are compiled in linux or mac os then uncomment the following
// line, otherwise comment it if you compile using visual studio in windows
#define _LINUX_MAC
// #define _OPENCV
...
void _Rlease2DBuffer(T** pBuffer, size_t nElements) {
  for (size_t i = 0; i < nElements; i++) delete[](pBuffer[i]);
  delete[] pBuffer;
  pBuffer = NULL;
}
